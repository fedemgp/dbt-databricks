...
class TestPythonNotebookUploader:
    ...
    def test_upload__with_notebook_scoped_packages(
        self, client, compiled_code, workdir, identifier, parsed_model
    ):
        client.workspace.create_python_model_dir.return_value = workdir
        parsed_model.catalog = "catalog"
        parsed_model.schema_ = "schema"
        ...
        parsed_model.config.packages = ["pandas", "numpy==1.24.0", "scikit-learn>=1.0"]

        uploader = PythonNotebookUploader(client, parsed_model)
        uploader.job_grants = {}
        uploader.notebook_access_control_list = []
        uploader.set_notebook_permissions = Mock()
        ...
        expected_code = f"%pip install {packages}\n\n# COMMAND ----------\ncompiled_code"
        client.workspace.upload_notebook.assert_called_once_with(file_path, expected_code)

        uploader.set_notebook_permissions.assert_not_called()

    def test_upload__without_notebook_scoped_packages(
        self, client, compiled_code, workdir, identifier, parsed_model
    ):
        client.workspace.create_python_model_dir.return_value = workdir
        parsed_model.catalog = "catalog"
        ...
        parsed_model.identifier = identifier
        parsed_model.config.notebook_scoped_libraries = False
        parsed_model.config.packages = ["pandas"]
        ...
        assert file_path == f"{workdir}{identifier}"
        # Should upload original code without packages
        client.workspace.upload_notebook.assert_called_once_with(file_path, compiled_code)
        uploader.set_notebook_permissions.assert_not_called()
